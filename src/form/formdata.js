...
router.post('/update', Validate, async (req, res) => {
    ...
    const query = `UPDATE formdata SET title = '${title}', description = '${description}',form = '${JSON.stringify(form)}' WHERE id = '${id}' AND userid='${req.authData.id}'`;
    connection.query(query, (err, results) => {
        console.log(results);
        if (err) {
            res.status(500).send({
...
router.post('/add', Validate, async (req, res) => {
    const query = "INSERT INTO formdata (id, form, title, description, userid, createdon) VALUES ($1,$2,$3,$4,$5,$6)";
    ...
    const values = [id, JSON.stringify(form), title, description, userid, createdon];
